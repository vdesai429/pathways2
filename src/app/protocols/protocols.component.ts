...
export class ProtocolsComponent implements OnInit {
  ...
  config: any = {
    toolbar: [
      'heading', '|', 'bold', 'italic', 'fontSize', 'fontFamily', 'fontColor', '|', 'bulletedList', 'numberedList', 'insertTable', 'imageUpload', 'mediaEmbed', '|', 'lineheight', 'undo', 'redo'
    ],
    table: {
...
catbackup = ['General', 'Professionalism', 'Contrast Administration', 'Results Communication']
  ...
  ngOnInit() {
    this.currentUser = this.kinvey.activeUser.data.first_name + ' ' + this.kinvey.activeUser.data.last_name;
    ...
    this.prevSub = this.kinvey.previousVersions.subscribe((entities) => {
      if (entities) {
          this.zone.run(() => {
            if (entities.length) {
              console.log(entities)
              this.previousVersions = entities;
          }
          else {
    ...
    this.editFileSub = this.kinvey.editfile.subscribe((element) => {
  ...
  onFileSelected(event) {
    ...
    for (let i = 0; i < event.target.files.length; i++) {
        ...
        file['modifyDate'] = new Date().toLocaleString();
        file['modifyBy'] = this.currentUser;
...
edit(element) {
  this.kinvey.getOneEdit(element._id);
  this.kinvey.getPrev(element._id);
}
...
public editorChanged2( { editor }: ChangeEvent ) {
...
saveCK() {
  ...
  if (this.editCKActive) {
   ...
   // console.log(index);
    console.log(this.fileArray[this.activeFileNum])
    this.fileArray[this.activeFileNum].data = this.editor;
    this.fileArray[this.activeFileNum].filename = this.editCKNameForm.value.filename;
    this.fileArray[this.activeFileNum].url = this.editCKNameForm.value.filename + '.html';
    this.fileArray[this.activeFileNum].modifyDate = new Date().toLocaleString();
    this.fileArray[this.activeFileNum].modifyBy = this.currentUser;
    this.editCKActive = false;
    this.kinvey.saveVersion(this.editElement._id, this.todaydate(), this.backupFile);
  }
...
editCK(filenum) {
  ...
  this.backupFile = Object.assign({}, this.activeFile);
...
openPrev(previousData) {
  console.log(previousData);
}

viewPrevious(file) {
  this.previousFile = file;
  this.editorContent2 = file.data;
  $('#ckModalPrev').modal({backdrop: 'static', keyboard: false});
}
